...
module.exports = function (app) {
    ...
    client.on('connection', function (socket) {
        socket.on('callAction', function(data){
            socket.broadcast.emit('callaction', data );
            ...
            switch (module){
                ...
                case 'DeleteMessageSelected':
                        Message.find({$or:[
                                ...
                                {'from': datam[2], 'to': datam[1], 'id': {$in: datam[3]}}]},function (err, result) {
                ...
                case 'updatemessageImage':
                    Message.update({'image': datam[2]}, {$set: {
                            'image': datam[1]
                        }}, function (err) {
                        if(err)
                            throw err;
                ...
                case 'messageSent':
                    User.find({'Email': datam[2].Email, 'chats.Id': datam[3].Email}, function(err, resp){
                        if(err)
                             throw err;
                        if(resp.length == 0){
                            User.update({'Email': datam[2].Email},{$push: {
                                'chats': {
                                    Id: datam[3].Email,
                                    time: getTime()[2],
                    ...
                    User.find({'Email': datam[3].Email, 'chats.Id': datam[2].Email}, function(err, resp){
                        if(err)
                            throw err;
                        if(resp.length == 0){
                            User.update({'Email': datam[3].Email},{$push: {
                                'chats': {
                                    Id: datam[2].Email,
                                    time: getTime()[2],
                                    Status: 'Add to friends',
                ...
                case 'friendAction':
                       var response;
                        switch (datam[1]){
                            case 'Add to friends':
                                User.update({'Email': datam[2], chats: {$elemMatch:{Id: datam[3]}}},{$set: {
                                    'chats.$.Status': 'Remove request'
                                        }}, function (err) {
                                    if(err)
                                         throw err;
                                });
                                User.update({'Email': datam[3], chats: {$elemMatch:{Id: datam[2]}}},{$set: {
                                        'chats.$.Status': 'Accept'
                                    }}, function (err) {
                                    if(err)
                                        throw err;
...
function socketResponse(socket, data,data1) {if(data1){
